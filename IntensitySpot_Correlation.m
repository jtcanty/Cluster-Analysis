% John Canty                                    % Created 09/08/15
% Yildiz Lab

% Extracts a list of drift-corrected coordinates and intensity values from
% a single-cell epifluorescence Z-stack. Also extracts the coordinates from
% analyzed telomere clusters from a STORM movie. Matches the coordinates
% and generates a correlation plot of spot intensity vs cluster size.

% Function calls:
% GaussFit.m - generates a bivariate Gaussian fit to the cluster

%%---------------Extract Z-stack Ints/Coords ---------------------
% Make sure you are in Z-stack working directory
clear all;

dirData = dir('*.txt');
num = length(dirData);
data_list = [];

for i = 1:num
    field = dirData(i);
    fname = field.name;
    if strfind(fname,'roi')
        info = tdfread(fname,'\t');
        ints = info.I;
        x_c = info.Xc; 
        y_c = info.Yc;
        data = horzcat(x_c,y_c,ints);
        data_list = vertcat(data_list,data);
    else
        continue
    end
end
 

%%----------------Extract centroid values --------------------------
% Navigate to folder containing clusters generated by STORM
clear dirData;
clear num;

folder = uigetdir;
cd(folder);
dirData = dir('*.txt');
num = length(dirData);

kmaxcoord_list = []
for i = 1:num
    field = dirData(i);
    fname = field.name;
    if strfind(fname,'output')
        info = dlmread(fname,'\t');
        [kmax,ind] = max(info(:,3));
        data = info(ind,:)
        kmaxcoord_list = vertcat(kmaxcoord_list,data(1:3));
    else
        continue
    end
end

%%-----------------------------------------------------------

numkmax = length(kmaxcoord_list)
display(numkmax)

numdata = length(data_list)
display(numdata)

pause

%%---------------Plot coordinates----------------------------
% Plot Z-stack coordinates
cf = figure(1)
set(cf,'Position',[10,10,1500,500])
s1 = subplot(1,3,1);
x1 = data_list(:,1);
y1 = data_list(:,2);
plot(x1,y1,'r');
title('Z-stack coordinates');
xlabel(s1,'X');
ylabel(s1,'Y');

% Plot STORM coordinates
s2 = subplot(1,3,2);
x2 = kmaxcoord_list(:,1);
y2 = kmaxcoord_list(:,2);
plot(x2,y2,'b');
title('STORM coordinates');
xlabel(s2,'X');
ylabel(s2,'Y');

% Plot both Z-stack and STORM coordinates
s3 = subplot(1,3,3);
plot(x1,y1,'r',x2,y2,'b');
title('Z-stack and STORM coordinates');
xlabel(s3,'X');
ylabel(s3,'Y');

pause

%%---------------Match coordinate values---------------------
% Calculate Euclidean distances for each point
% sz_int = [x,y,intensity, cluster size] 
sz_int = []

% Iterate over Z-stack coordinates
for i = 1:numdata
    % norm array of Z-stack coord to all STORM coords
    norm = sqrt((kmaxcoord_list(:,1)-data_list(i,1)).^2 + (kmaxcoord_list(:,2)-data_list(i,2)).^2)
    % find min norm
    [min_norm,ind] = min(norm)
    if min_norm < 3
        data = horzcat(data_list(i,:),kmaxcoord(ind,3))
        sz_int = vertcat(sz_int,data)
        % Add functionality to remove rows
    else
        continue
    end
end

%%---------------Generate correlation plot--------------------------

